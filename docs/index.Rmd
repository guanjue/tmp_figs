...
write.table(cbind(target_ct_rep1_i[,1], round(target_ct_rep1_i_sig_JMnorm_linear, 3)), 'TCD8.JMnorm_sigmat.txt', quote=F, col.names=F, row.names = F, sep='\t')
```
...
pheatmap(file_ref_sig0_log2_cor, color=my_colorbar, breaks = breaksList, cluster_rows = F, cluster_cols = F, show_rownames = F, show_colnames = T, cex=1.2, main='Reference Corr matrix')
```
...
# plot the correlation matrix of reference cell-type
plot(as.numeric(file_ref_sig0_log2_cor), as.numeric(target_ct_rep1_i_sig_JMnorm_cor), xlab='Reference', ylab='Target', xlim=c(-0.5,1), ylim=c(-0.5,1), main='After JMnorm', cex.axis=1.5)
abline(0,1)

plot(as.numeric(file_ref_sig0_log2_cor), as.numeric(target_ct_rep1_i_sig_cor), xlab='Reference', ylab='Target', xlim=c(-0.5,1), ylim=c(-0.5,1), main='Before JMnorm', cex.axis=1.5)
abline(0,1)
end.time <- Sys.time()
```
